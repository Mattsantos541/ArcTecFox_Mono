...
1. Organize tasks into the following standard groups for readability:
...
2. *Mandatory Parent-Asset Oversight Tasks (include at least these two):*
   ...
   Add additional parent-level systemic checks if best practice applies (e.g., controls/PLC status, system alarms review, utilities, safety interlocks, vibration across assemblies, housekeeping, corrosion survey).

3. For *every task* (parent-only scope), provide *all* fields below. If a field is not applicable, return "Not applicable". Do not omit fields. If user input is insufficient, reference the *manufacturer manual* or credible standards; cite them:
   ...
   * "citations": Prefer the manufacturer's manual; else standards (ISO/ASTM/API) or reputable supplier data (SKF, Mobil, Shell).
...
4. *Critical Spare Parts for the Parent Asset:*
   ...
   If a field is unknown from user input, infer from the manufacturer's manual or reliable sources and cite them.
